...
class DailyFinancialClosingPage extends StatefulWidget {
  ...
  @override
  State<DailyFinancialClosingPage> createState() => _DailyFinancialClosingPageState();
}
...
class _DailyFinancialClosingPageState extends State<DailyFinancialClosingPage> {
  ...
  Future<DailyFinancialClosingDraft> _loadPreview() async {
    ...
    final List<SupplierRecharge> recharges = await widget.financeRepository.watchSupplierRecharges().first;
    ...
    final List<CommissionEntry> commissions = await widget.commissionRepository.watchCommissions().first;
  ...
  Future<void> _closeDay(DailyFinancialClosingDraft preview, List<DailyFinancialClosing> history) async {
    if (history.any((DailyFinancialClosing item) => item.dateKey == preview.dateKey)) {
      _message('La journée est déjà clôturée.');
      return;
    }
    final TextEditingController actual = TextEditingController(text: '${preview.waveTheoreticalBalance.clamp(0, 1000000000)}');
      ...
      builder: (BuildContext dialogContext) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Text('Wave théorique : ${formatCfaFull(preview.waveTheoreticalBalance)}'),
            const SizedBox(height: 10),
            TextField(controller: actual, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Solde Wave réellement constaté')),
            const SizedBox(height: 10),
            TextField(controller: differenceNote, maxLines: 2, decoration: const InputDecoration(labelText: 'Justification de l’écart (si nécessaire)')),
            const SizedBox(height: 10),
            const Text('La clôture sera enregistrée comme un instantané immuable. Tout écart Wave doit être justifié et les corrections ultérieures apparaîtront dans les mouvements du jour suivant.'),
          ]),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Annuler')),
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Clôturer')),
        ],
      ),
    );
    ...
    if (difference != 0 && cleanedDifferenceNote.length < 3) {
      _message('Justifie l’écart entre le solde Wave réel et le solde théorique.');
      return;
    }
    final DailyFinancialClosingDraft draft = DailyFinancialClosingDraft(
      ...
      waveDifferenceNote: cleanedDifferenceNote.isEmpty ? null : cleanedDifferenceNote,
    );
    setState(() => _busy = true);
    try {
      await widget.financeRepository.createDailyClosing(draft: draft, staffId: widget.user.id, staffName: widget.user.name);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Clôture journalière', style: TextStyle(fontSize: IzyTelTypeScale.title3, fontWeight: FontWeight.w800)), actions: [IconButton(onPressed: _busy ? null : _reload, icon: const Icon(Symbols.refresh_rounded))]),
      body: StreamBuilder<List<DailyFinancialClosing>>(
        stream: widget.financeRepository.watchDailyClosings(),
        builder: (BuildContext context, AsyncSnapshot<List<DailyFinancialClosing>> historySnapshot) {
          final List<DailyFinancialClosing> history = historySnapshot.data ?? const <DailyFinancialClosing>[];
          return FutureBuilder<DailyFinancialClosingDraft>(
            future: _previewFuture,
            builder: (BuildContext context, AsyncSnapshot<DailyFinancialClosingDraft> previewSnapshot) {
              if (!previewSnapshot.hasData && previewSnapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
              if (previewSnapshot.hasError) return Center(child: Padding(padding: const EdgeInsets.all(24), child: Text('Impossible de préparer la clôture : ${previewSnapshot.error}', textAlign: TextAlign.center)));
              final DailyFinancialClosingDraft preview = previewSnapshot.data!;
              final bool alreadyClosed = history.any((DailyFinancialClosing item) => item.dateKey == preview.dateKey);
              return ListView(padding: const EdgeInsets.all(20), children: [
                Row(children: [
                  Expanded(child: FinancialMetricCard(label: 'Encaissements', value: formatCfa(preview.clientReceipts), icon: Symbols.call_received_rounded, accent: IzyTelColors.success, caption: '${preview.successfulOrdersCount} réussite(s)')),
                  const SizedBox(width: 10),
                  Expanded(child: FinancialMetricCard(label: 'Bénéfice estimé', value: formatCfa(preview.estimatedProfit), icon: Symbols.trending_up_rounded, accent: preview.estimatedProfit >= 0 ? IzyTelColors.success : IzyTelColors.error)),
                ]),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: FinancialMetricCard(label: 'Wave théorique', value: formatCfa(preview.waveTheoreticalBalance), icon: Symbols.account_balance_wallet_rounded, accent: IzyTelColors.wave)),
                  const SizedBox(width: 10),
                  Expanded(child: FinancialMetricCard(label: 'Créances clients', value: formatCfa(preview.customerReceivables), icon: Symbols.request_quote_rounded, accent: IzyTelColors.warning)),
                ]),
                const SizedBox(height: 16),
                _Section(title: 'Activité du jour', rows: [
                  _Row('Commandes réussies', preview.successfulOrdersAmount),
                  _Row('Recharges fournisseurs · principal', preview.supplierRechargePrincipal),
                  _Row('Bonus fournisseurs', preview.supplierRechargeBonus),
                  _Row('Stock réseau reçu', preview.supplierRechargeReceived),
                  _Row('Paiements fournisseurs', preview.supplierPayments, outgoing: true),
                  _Row('Crédits créés', preview.creditsCreated),
                  _Row('Crédits encaissés', preview.creditSettlements),
                  _Row('Dépenses', preview.expenses, outgoing: true),
                  _Row('Remboursements', preview.refunds, outgoing: true),
                  _Row('Commissions générées', preview.commissionsEarned, outgoing: true),
                  _Row('Commissions payées', preview.commissionsPaid, outgoing: true),
                ]),
                const SizedBox(height: 12),
                _Section(title: 'Position à la clôture', rows: [
                  _Row('Orange disponible', preview.orangeAvailable),
                  ...
                  _Row('Dette fournisseurs', preview.supplierDebt, outgoing: true),
                  _Row('Commissions dues', preview.commissionDebt, outgoing: true),
                ]),
                const SizedBox(height: 14),
                SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _busy || alreadyClosed ? null : () => _closeDay(preview, history), icon: Icon(alreadyClosed ? Symbols.check_circle_rounded : Symbols.lock_rounded), label: Text(alreadyClosed ? 'Journée déjà clôturée' : 'Clôturer aujourd’hui'))),
                const SizedBox(height: 22),
                const Text('Historique', style: TextStyle(fontWeight: FontWeight.w800, color: IzyTelColors.textPrimary)),
                const SizedBox(height: 9),
                if (history.isEmpty)
                  const FinanceEmptyState(icon: Symbols.calendar_month_rounded, title: 'Aucune clôture', message: 'La première clôture apparaîtra ici.')
                else
                  ...history.take(30).map((DailyFinancialClosing item) => Container(
                    ...
                    child: Row(children: [
                      const Icon(Symbols.lock_rounded, color: IzyTelColors.primary, size: 20),
                      const SizedBox(width: 10),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.dateKey, style: const TextStyle(fontWeight: FontWeight.w700)), Text(item.waveDifference == 0 ? 'Wave ${formatCfa(item.waveActualBalance)} · aucun écart' : 'Wave ${formatCfa(item.waveActualBalance)} · écart ${formatCfa(item.waveDifference)}${item.waveDifferenceNote?.trim().isNotEmpty == true ? ' · ${item.waveDifferenceNote}' : ''}', style: const TextStyle(fontSize: 10.5, color: IzyTelColors.textMuted))])),
                      Text(formatCfa(item.estimatedProfit), style: TextStyle(fontWeight: FontWeight.w800, color: item.estimatedProfit >= 0 ? IzyTelColors.success : IzyTelColors.error)),
                    ]),
                  )),
              ]);
            },
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(fontWeight: FontWeight.w800, color: IzyTelColors.textPrimary)),
        const SizedBox(height: 10),
        ...rows.map((row) => Padding(padding: const EdgeInsets.only(bottom: 7), child: Row(children: [Expanded(child: Text(row.label, style: const TextStyle(fontSize: 12, color: IzyTelColors.textSecondary))), Text('${row.outgoing ? '-' : ''}${formatCfa(row.amount)}', style: TextStyle(fontWeight: FontWeight.w700, color: row.outgoing ? IzyTelColors.warning : IzyTelColors.textPrimary))]))),
      ]),
    );
  }
}
