...
class WaveFinanceCalculator {
  ...
  static WaveCashSnapshot calculate({
    ...
    final DateTime effectiveAt =
        opening?.effectiveAt ?? DateTime.fromMillisecondsSinceEpoch(0);
    final int clientPayments = orders
    ...
    final int creditIncome = creditSettlements
        .where((CustomerCreditSettlement value) {
          return value.channel == FinancePaymentChannel.wave &&
              !value.paidAt.isBefore(effectiveAt);
        })
        .fold<int>(
          0,
          (int total, CustomerCreditSettlement value) => total + value.amount,
        );

    final int supplierOut = supplierPayments
        .where((SupplierPayment value) {
          return value.channel == FinancePaymentChannel.wave &&
              !value.paidAt.isBefore(effectiveAt);
        })
        .fold<int>(
          0,
          (int total, SupplierPayment value) => total + value.amount,
        );

    final int refundOut = refunds
        .where((RefundCase value) {
          final DateTime? date = value.refundedAt;
          return value.paymentChannel == 'wave' &&
              date != null &&
              !date.isBefore(effectiveAt);
        })
        .fold<int>(0, (int total, RefundCase value) => total + value.amount);

    final int expenseOut = expenses
        .where((FinanceExpense value) {
          return value.channel == FinancePaymentChannel.wave &&
              !value.spentAt.isBefore(effectiveAt);
        })
        .fold<int>(
          0,
          (int total, FinanceExpense value) => total + value.amount,
        );

    final int commissionOut = commissionPayouts
        .where((CommissionPayout value) {
          return value.paymentChannel == 'wave' &&
              !value.paidAt.isBefore(effectiveAt);
        })
        .fold<int>(
...
class WorkingCapitalCalculator {
  ...
  static WorkingCapitalSnapshot calculate({
    ...
    final int available = networkFunds.values.fold<int>(
      0,
      (int total, NetworkFundSnapshot item) => total + item.available,
    );
    final int committed = networkFunds.values.fold<int>(
      0,
      (int total, NetworkFundSnapshot item) => total + item.committed,
    );
    final int freeNetwork = networkFunds.values.fold<int>(0, (
      int total,
      NetworkFundSnapshot item,
    ) {
      return total + math.max(0, item.available - item.committed);
    });
    final int supplierDebt = supplierAccounts.fold<int>(
      0,
      (int total, SupplierAccount item) => total + math.max(0, item.balance),
    );
    final int receivables = credits.fold<int>(
      0,
      (int total, CustomerCredit item) => total + math.max(0, item.outstanding),
    );
    final int commissionDebt = commissionAccounts.fold<int>(
      0,
      (int total, CommissionAccount item) => total + math.max(0, item.balance),
    );
    ...
    return WorkingCapitalSnapshot(
      ...
      netWorkingCapital:
          liquidity + receivables - supplierDebt - commissionDebt,
    );
  }
}
...
class DailyClosingCalculator {
  ...
  static DailyClosingComputation calculate({
    ...
    final List<QueueOrder> paidToday = orders
        .where((QueueOrder order) {
          final DateTime? date = order.paymentConfirmedAt ?? order.paidAt;
          return order.paymentStatus == OrderPaymentStatus.confirmed &&
              date != null &&
    ...
    final List<NetworkTransaction> successToday = networkTransactions
        .where((NetworkTransaction item) {
    ...
    final List<SupplierRecharge> rechargeToday = recharges
        .where((SupplierRecharge item) => isSameDay(item.createdAt, day))
        .toList(growable: false);
    final int supplierPaymentToday = supplierPayments
        .where((SupplierPayment item) => isSameDay(item.paidAt, day))
        .fold<int>(0, (int total, SupplierPayment item) => total + item.amount);
    final int creditCreatedToday = credits
        .where((CustomerCredit item) => isSameDay(item.createdAt, day))
        .fold<int>(0, (int total, CustomerCredit item) => total + item.amount);
    final int settlementToday = settlements
        .where((CustomerCreditSettlement item) => isSameDay(item.paidAt, day))
        .fold<int>(
          0,
          (int total, CustomerCreditSettlement item) => total + item.amount,
        );
    final int expenseToday = expenses
        .where((FinanceExpense item) => isSameDay(item.spentAt, day))
        .fold<int>(0, (int total, FinanceExpense item) => total + item.amount);
    final List<RefundCase> refundsToday = refunds
        .where(
          (RefundCase item) =>
              item.refundedAt != null && isSameDay(item.refundedAt!, day),
        )
        .toList(growable: false);
    ...
    final int profitRefundToday = refundsToday
        .where(
          (RefundCase item) =>
              historicallySuccessfulOrderIds.contains(item.orderId),
        )
        .fold<int>(0, (int total, RefundCase item) => total + item.amount);
    final int commissionEarnedToday = commissions
        .where((CommissionEntry item) => isSameDay(item.earnedAt, day))
        .fold<int>(
          0,
          (int total, CommissionEntry item) => total + item.commissionAmount,
        );
    final int commissionPaidToday = commissionPayouts
        .where((CommissionPayout item) => isSameDay(item.paidAt, day))
        .fold<int>(
          0,
          (int total, CommissionPayout item) => total + item.amount,
        );
    ...
    for (final SupplierRecharge recharge in recharges) {
      totalRechargeReceived[recharge.network] =
          totalRechargeReceived[recharge.network]! + recharge.receivedAmount;
    ...
    for (final NetworkTransaction movement in successToday) {
      ...
      final double ratio = received <= 0
          ? 1
          : (cost / received).clamp(0, 1).toDouble();
      estimatedNetworkCost += (movement.amount * ratio).round();
    }

    final int successAmount = successToday.fold<int>(
      0,
      (int total, NetworkTransaction item) => total + item.amount,
    );
    ...
    return DailyClosingComputation(
      clientReceipts: paidToday.fold<int>(
        0,
        (int total, QueueOrder item) => total + item.amount,
      ),
      successfulOrdersCount: successToday.length,
      successfulOrdersAmount: successAmount,
      supplierRechargePrincipal: rechargeToday.fold<int>(
        0,
        (int total, SupplierRecharge item) => total + item.principalAmount,
      ),
      supplierRechargeBonus: rechargeToday.fold<int>(
        0,
        (int total, SupplierRecharge item) => total + item.bonusAmount,
      ),
      supplierRechargeReceived: rechargeToday.fold<int>(
        0,
        (int total, SupplierRecharge item) => total + item.receivedAmount,
      ),
      supplierPayments: supplierPaymentToday,
      creditsCreated: creditCreatedToday,
      creditSettlements: settlementToday,
      customerReceivables: credits.fold<int>(
        0,
        (int total, CustomerCredit item) =>
            total + math.max(0, item.outstanding),
      ),
      expenses: expenseToday,
      refunds: refundToday,
      commissionsEarned: commissionEarnedToday,
      commissionsPaid: commissionPaidToday,
      supplierDebt: supplierAccounts.fold<int>(
        0,
        (int total, SupplierAccount item) => total + math.max(0, item.balance),
      ),
      commissionDebt: commissionAccounts.fold<int>(
        0,
        (int total, CommissionAccount item) =>
            total + math.max(0, item.balance),
      ),
