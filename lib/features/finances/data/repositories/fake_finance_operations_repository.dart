...
class FakeFinanceOperationsRepository implements FinanceOperationsRepository {
  ...
  Stream<List<SupplierAccount>> watchSupplierAccounts() async* {
    ...
    yield* _changes.stream.map(
      (_) => List<SupplierAccount>.unmodifiable(_supplierAccounts),
    );
  }
  ...
  Stream<List<SupplierRecharge>> watchSupplierRecharges() async* {
    yield _sortedByDate(
      _recharges,
      (SupplierRecharge value) => value.createdAt,
    );
    yield* _changes.stream.map(
      (_) => _sortedByDate(
        _recharges,
  ...
  Stream<List<SupplierPayment>> watchSupplierPayments() async* {
    yield _sortedByDate(
      _supplierPayments,
      (SupplierPayment value) => value.paidAt,
    );
    yield* _changes.stream.map(
      (_) => _sortedByDate(
        _supplierPayments,
  ...
  Stream<List<CustomerCredit>> watchCustomerCredits() async* {
    ...
    yield* _changes.stream.map(
      (_) => _sortedByDate(_credits, (CustomerCredit value) => value.updatedAt),
    );
  }

  @override
  Stream<List<CustomerCreditSettlement>>
  watchCustomerCreditSettlements() async* {
    yield _sortedByDate(
      _settlements,
      (CustomerCreditSettlement value) => value.paidAt,
    );
    yield* _changes.stream.map(
      (_) => _sortedByDate(
        _settlements,
  ...
  Stream<List<FinanceExpense>> watchExpenses() async* {
    ...
    yield* _changes.stream.map(
      (_) => _sortedByDate(_expenses, (FinanceExpense value) => value.spentAt),
    );
  }
  ...
  Stream<List<WaveBalanceAdjustment>> watchWaveBalanceAdjustments() async* {
    yield _sortedByDate(
      _waveAdjustments,
      (WaveBalanceAdjustment value) => value.effectiveAt,
    );
    yield* _changes.stream.map(
      (_) => _sortedByDate(
        _waveAdjustments,
  ...
  Stream<List<DailyFinancialClosing>> watchDailyClosings() async* {
    yield _sortedByDate(
      _closings,
      (DailyFinancialClosing value) => value.closedAt,
    );
    yield* _changes.stream.map(
      (_) => _sortedByDate(
        _closings,
  ...
  Future<String> recordSupplierRecharge({
    ...
    if (draft.amountOwed != draft.principalAmount) {
      throw ArgumentError(
        'Une recharge crée d’abord la dette complète du principal.',
      );
    }
    final DateTime now = DateTime.now();
  ...
  Future<String> recordSupplierPayment({
    ...
    final SupplierAccount? current = _account(draft.supplierId);
    if (current == null || draft.amount > current.balance) {
      throw StateError('Le règlement dépasse le solde fournisseur.');
    }
    final DateTime now = DateTime.now();
  ...
  Future<String> createCustomerCredit({
    ...
    if (_credits.any((CustomerCredit item) => item.orderId == draft.orderId)) {
  ...
  Future<String> settleCustomerCredit({
    ...
    final int index = _credits.indexWhere(
      (CustomerCredit item) => item.id == creditId,
    );
    if (index < 0) {
      throw StateError('Crédit client introuvable.');
    }
    final CustomerCredit current = _credits[index];
    if (amount <= 0 || amount > current.outstanding) {
      throw StateError('Montant de règlement invalide.');
    }
    final DateTime now = DateTime.now();
    ...
    _credits[index] = CustomerCredit(
      ...
      paidAmount: nextPaid,
      status: settled
          ? CustomerCreditStatus.settled
          : CustomerCreditStatus.partial,
      note: current.note,
      createdAt: current.createdAt,
      createdBy: current.createdBy,
      createdByName: current.createdByName,
  ...
  Future<void> setWaveOpeningBalance({
    ...
    _waveAdjustments.add(
      WaveBalanceAdjustment(
        id: adjustmentId,
  ...
  Future<void> createDailyClosing({
    ...
    if (_closings.any(
      (DailyFinancialClosing item) => item.dateKey == draft.dateKey,
    )) {
      throw StateError('Cette journée a déjà été clôturée.');
    }
    if (draft.waveActualBalance < 0) {
      throw ArgumentError('Le solde Wave réel est invalide.');
    }
    if (draft.waveDifference != 0 &&
        (draft.waveDifferenceNote?.trim().length ?? 0) < 3) {
      throw ArgumentError('Un écart Wave doit être justifié.');
    }
    final DateTime now = DateTime.now();
    _closings.add(
      DailyFinancialClosing(
        id: draft.dateKey,
  ...
  void _upsertAccount(SupplierAccount account) {
    final int index = _supplierAccounts.indexWhere(
      (SupplierAccount item) => item.supplierId == account.supplierId,
    );
    if (index < 0) {
  ...
  List<FinanceSupplier> _sortedSuppliers() {
    final List<FinanceSupplier> result = List<FinanceSupplier>.from(_suppliers)
      ..sort(
        (FinanceSupplier a, FinanceSupplier b) =>
