...
void main() {
  ...
  test('recharge fournisseur écrit stock, dette et mouvement réseau dans une transaction', () {
    final int start = source.indexOf('Future<String> recordSupplierRecharge');
    final int end = source.indexOf('Future<String> recordSupplierPayment', start);
  ...
  test('crédit client finance atomiquement la commande sans simuler un paiement Wave', () {
    final int start = source.indexOf('Future<String> createCustomerCredit');
    final int end = source.indexOf('Future<String> settleCustomerCredit', start);
    expect(start, greaterThanOrEqualTo(0));
    expect(end, greaterThan(start));
    final String block = source.substring(start, end);
  ...
  test('recalage Wave est journalisé dans une transaction avec le nouveau point de départ', () {
